...
- **RAM:** 16 GB minimum (project is tuned for a 16 GB ThinkPad T14s — 6 GB Docker cap + ~1.5 GB local supervisor + OS overhead)
- **CPU:** No dedicated GPU required — integrated graphics is fine; local models are kept ≤8B params for CPU-usable speed
- **Docker:** Required, with resource limits enforced (`--memory=6g --cpus=3`)
- **Node.js:** Required for the OpenClaw background daemon
- **Accounts/keys needed:**
  - Google Gemini API key (free tier)
  - Kaggle account (for DeepSeek GPU hosting via Cloudflare Tunnel)
  - Azure Student VM (or equivalent always-on host for the task queue backend)
